...
import ESpeakTTS from './lib/espeak';
...
export default function App() {
  ...
  const speakText = async (text: string, id: string | null = null) => {
    ...
    showToast('Generating audio (eSpeak-ng)...');
    ...
    try {
      ...
      const playNext = async () => {
        ...
        if (!/[a-zA-Z\u0980-\u09FF0-9]/.test(chunkText)) {
          chunkIndex++;
          playNext();
          return;
        }

        const isBangla = /[\u0980-\u09FF]/.test(chunkText);
        const lang = isBangla ? 'bn' : 'en';
        
        const wavData = await ESpeakTTS.generateWav(chunkText, lang);
